...
class TestFeatures(TestCase):
    ...
    def test_standard_scaler_custom(self):
        data = [[2., 4., 6., 8.], [12., 14., 16., 18.], [22., 24., 26., 28.]]
        expected = np.array([[-0.85732141 ,-0.85732141, -0.85732141, -0.85732141]])
        scaler = StandardScaler()
        scaler.fit(data)
        result = scaler.transform([[5.,7.,9.,11.]]) 
        assert np.allclose(result, expected, atol=1e-6), "Scaler transform does not return expected values. Expect {}. Got: {}".format(expected.reshape(1,-1), result.reshape(1,-1))
